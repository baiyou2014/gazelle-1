...
<div class="thin">
  ...
  <form class="search_form" name="transcodes" action="" method="get">
    ...
    <table cellpadding="6" cellspacing="1" border="0" class="border" width="100%">
      <tr>
        ...
        <td>
          <select name="filter">
            <option value="all" <?=($_GET['filter'] == 'all' ? ' selected="selected"' : '')?>>Show
              all torrents</option>
            <option value="snatched" <?=($_GET['filter'] == 'snatched' ? ' selected="selected"' : '')?>>Snatched
              only</option>
            <option value="seeding" <?=($_GET['filter'] == 'seeding' ? ' selected="selected"' : '')?>>Seeding
              only</option>
            <option value="uploaded" <?=($_GET['filter'] == 'uploaded' ? ' selected="selected"' : '')?>>Uploaded
              only</option>
          </select>
          <select name="target">
            <option value="any" <?=($_GET['target'] == 'any' ? ' selected="selected"' : '')?>>Any
              transcode missing</option>
            <option value="v0" <?=($_GET['target'] == 'v0' ? ' selected="selected"' : '')?>>V0
              missing</option>
            <option value="v2" <?=($_GET['target'] == 'v2' ? ' selected="selected"' : '')?>>V2
              missing</option>
            <option value="320" <?=($_GET['target'] == '320' ? ' selected="selected"' : '')?>>320
              missing</option>
            <option value="all" <?=($_GET['target'] == 'all' ? ' selected="selected"' : '')?>>All
              transcodes missing</option>
          </select>
        </td>
      </tr>
      <tr>
  ...
  <div class="box pad" style="padding: 10px 10px 10px 20px;">
    <p>
      This page aims at listing <?=TORRENTS_PER_PAGE?> random
      transcodable perfect FLACs matching the options you selected above, but there can be more or less matches on this
      page. The following numbers tell you something about the torrents currently listed below and can change if you
      reload.<br /><br />
      ...
      Number of missing V2 / V0 / 320 transcodes: <?=number_format($Counter['miss_V2 (VBR)'])?> /
      <?=number_format($Counter['miss_V0 (VBR)'])?> /
      <?=number_format($Counter['miss_320'])?>
    </p>
  </div>
  <h3>List</h3>
...
if ($ResultCount == 0) {
    ...
    <tr<?=($Edition['FLACIsSnatched'] ? ' class="snatched_torrent"' : '')?>>
      <td>
        ...
        <?=$DisplayName?>
        <div class="torrent_info"><?=$Edition['EditionName']?>
        </div>
        <div class="tags"><?=$TorrentTags->format('better.php?action=transcode&tags=')?>
        </div>
      </td>
      <td><?=(isset($Edition['MP3s']['V2 (VBR)']) ? '<strong class="important_text_alt">YES</strong>' : '<strong class="important_text">NO</strong>')?>
      </td>
      <td><?=(isset($Edition['MP3s']['V0 (VBR)']) ? '<strong class="important_text_alt">YES</strong>' : '<strong class="important_text">NO</strong>')?>
      </td>
      <td><?=(isset($Edition['MP3s']['320']) ? '<strong class="important_text_alt">YES</strong>' : '<strong class="important_text">NO</strong>')?>
      </td>
      </tr>
      <?php
            }
